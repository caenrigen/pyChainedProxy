...
try:
    ...
    def SSL_Connect(ctx, sock, server_side=False, accepted=False, connected=False, verify_names=None):
        if DEBUG:
        ...
        if verify_names:
            ...
            def vcb(conn, x509, errno, depth, rc):
                ...
                if depth != 0:
                    return True
                return SSL_CheckName(x509.get_subject().commonName.lower(), x509.digest("sha1"), verify_names) > 0
            ...
            if "subjectAltName" in cert:
                for field in cert["subjectAltName"]:
                    if field[0].lower() == "dns":
                        name = field[1].lower()
                        valid += SSL_CheckName(field[1].lower(), certhash, names)
            return valid > 0
...
PROXY_SSL_TYPES = (PROXY_TYPE_SSL, PROXY_TYPE_SSL_WEAK, PROXY_TYPE_SSL_ANON, PROXY_TYPE_HTTPS, PROXY_TYPE_HTTPS_CONNECT)
...
def addproxy(dest, proxytype=None, addr=None, port=None, rdns=True, username=None, password=None, certnames=None):
    global _proxyroutes
...
def usesystemdefaults():
    ...
    no_proxy.extend(os.environ.get("NO_PROXY", os.environ.get("NO_PROXY", "")).split(","))
...
class socksocket(socket.socket):
    ...
    def __init__(self, family=socket.AF_INET, type=socket.SOCK_STREAM, proto=0, *args, **kwargs):
        ...
        self.__sock = _orgsocket(family, self.__type, self.__proto, *self.__args, **self.__kwargs)
        self.__proxy = None
    ...
    def __setattr__(self, name, value):
        if name.startswith("_socksocket__"):
            ...
            return setattr(object.__getattribute__(self, "_socksocket__sock"), name, value)
    ...
    def __negotiatesocks5(self, destaddr, destport, proxy):
        ...
        if chosenauth[1:2] == chr(0x00).encode():
            ...
            if six.PY2:
                ...
                self.sendall(
                    chr(0x01).encode()
                    + chr(len(proxy[P_USER])).encode()
                    + proxy[P_USER].encode()
                    + chr(len(proxy[P_PASS])).encode()
                    + proxy[P_PASS].encode()
                )
        ...
        try:
            ...
            if proxy[P_RDNS]:
                # Resolve remotely
                ipaddr = None
                req = req + (chr(0x03).encode() + chr(len(destaddr)).encode() + six.b(destaddr))
            else:
                # Resolve locally
    ...
    def __negotiatehttpproxy(self):
        ...
        if buf[0:3].lower() not in ("get", "pos", "hea", "put", "del", "opt", "pro"):
            if DEBUG:
                DEBUG("*** Not HTTP, failing back to HTTP CONNECT.")
    ...
    def __negotiatessl(self, destaddr, destport, proxy, weak=False, anonymous=False):
        ...
        try:
            ...
            self.__sock.setblocking(1)
            self.__sock = SSL_Connect(ctx, self.__sock, connected=True, verify_names=want_hosts)
        except:
            if DEBUG:
                DEBUG("*** SSL problem: %s/%s/%s" % (sys.exc_info(), self.__sock, want_hosts))
            raise
    ...
    def __do_connect(self, addrspec):
        if ":" in addrspec[0]:
            self.__sock = _orgsocket(socket.AF_INET6, self.__type, self.__proto, *self.__args, **self.__kwargs)
            return self.__sock.connect(addrspec)
        else:
            try:
                self.__sock = _orgsocket(socket.AF_INET, self.__type, self.__proto, *self.__args, **self.__kwargs)
                return self.__sock.connect(addrspec)
